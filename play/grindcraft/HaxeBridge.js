...
window.addEventListener('blur', function(){ stateManager.onBlur();} );
window.addEventListener('focus', function(){ stateManager.onFocus(); });
window.addEventListener('mousemove', function(){ stateManager.onMouseMove(); });
window.addEventListener('click', function(){ stateManager.onMouseMove(); });
window.addEventListener('touchstart', function(){ stateManager.onMouseMove(); });
window.addEventListener('touchend', function(){ stateManager.onMouseMove(); });
...
class globalAdManager
{
	...
	msecToNextAd()
	{
		...
		{
			return 1000 * 60 * 10; // at 15min
		}
		else if(this.numAdsShownInSession == 2)
		{
			return 1000 * 60 * 15; // at 30min
		}
		else
		{
			return 1000 * 60 * 20; // at 35min & repeat every 15min
			// Split test this to see whether 10, 12, or 15 leads to more total ads
		}
	}
...
setInterval(function(){ adManager.update(); }, 1000);
setInterval(function(){ 
	if(localStorage.hasOwnProperty("gc2Clicked"))
	{
		adManager.startGrindcraft2AdOverlay();
		localStorage.removeItem("gc2Clicked");
	}
}, 50);
